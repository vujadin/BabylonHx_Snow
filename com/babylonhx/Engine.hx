...
@:expose('BABYLON.Engine') class Engine {
	...
	private var _canvasClientRect:Dynamic = { x: 0, y: 0, width: 800, height: 600 };
	...
	public function createTexture(url:String, noMipmap:Bool, invertY:Bool, scene:Scene, samplingMode:Int = Texture.TRILINEAR_SAMPLINGMODE, onLoad:Void->Void = null, onError:Void->Void = null, buffer:Dynamic = null):BabylonTexture {
		...
		if (isTGA) {
			...
			var onload = function(img:Dynamic) {
				prepareTexture(texture, GL, scene, img.image.width, img.image.height, invertY, noMipmap, false, function(potWidth:Int, potHeight:Int) {
	...
	public function createCubeTexture(rootUrl:String, scene:Scene, extensions:Array<String> = null, noMipmap:Bool = false):BabylonTexture {
		...
		if (isDDS) {
			...
			function generate() {
				var width = Tools.GetExponantOfTwo(imgs[0].image.width, this._caps.maxCubemapTextureSize);
				var height = width;
			...
			function loadImage() {
				Tools.LoadImage(rootUrl + extensions[i], function(bd:AssetImage) {
					imgs.push(bd);
